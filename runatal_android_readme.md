...
**Runatal** adheres to **Material 3 Expressive**, Google's latest evolution of Material Design launched in May 2025. This design system emphasizes emotional engagement, fluid motion, and personality-driven interfaces while maintaining usability and accessibility.
...
### 3.2 Core Principles for Runatal

#### 3.2.1 Typography System

Runatal uses the M3 Expressive typography scale with **Roboto Flex** as the primary typeface:
...
**Runatal adheres to a monochromatic black and white color scheme** with strategic use of transparency, tints, and gradients to create depth and visual hierarchy. This minimalist approach emphasizes the ancient, timeless nature of runic scripts while maintaining M3 Expressive's emotional expressiveness.
...
### 3.3 Expressive Components for Runatal
...
### 3.6 Why M3 Expressive + Monochrome for Runatal?

Material 3 Expressive with a black & white color scheme aligns perfectly with the Runatal app vision:
...
### 5.1 Architecture
- `RunatalWidget` (GlanceAppWidget)
- `RunatalWidgetReceiver`
- `RunatalWidgetEntry`
- `RunatalWidgetContent`
...
android {
    namespace = "com.runatal.android"
    compileSdk = 34

    defaultConfig {
        applicationId = "com.runatal.android"
        minSdk = 26
        targetSdk = 34
        versionCode = 1
        versionName = "1.0.0"
...
#### `proguard-rules.pro`
```proguard
# Runatal specific rules

# Keep custom fonts
-keep class **.R$font { *; }

# Keep Room entities
-keep class com.runatal.android.data.local.entity.** { *; }
...
-keep class com.runatal.android.widget.** { *; }
...
│   ├── src/
│   │   ├── main/
│   │   │   ├── java/com/runatal/android/
│   │   │   │   ├── RunatalApp.kt                   # Application class
...
- [ ] Create `@HiltAndroidApp` application class in `RunatalApp.kt`
...
- [ ] Create `widget/RunatalWidget.kt` (GlanceAppWidget)
- [ ] Create `widget/RunatalWidgetReceiver.kt` (GlanceAppWidgetReceiver)
- [ ] Create `widget/RunatalWidgetContent.kt` (@Composable for Glance)
